...
インストール後、ターミナルを再起動して `uv --version` でバージョンが表示されれば完了です。
...
インストール後、VS Code を再起動するか、コマンドパレット（`Cmd+Shift+P`）から `Developer: Reload Window` を実行します。

### 2. Claude Code パネルを開く

左サイドバーに Claude Code のアイコン（Anthropicロゴ）が表示されます。クリックしてパネルを開きます。

または、コマンドパレット（`Cmd+Shift+P`）から以下を選択します：
